...
- Latest staging release is commit `2f4b393` (`feat(i18n): Indonesian drafts for existing CMS content`); GitHub Actions run `36034583065` built, migrated and deployed; its smoke test got a 403 from Hostinger's CDN against the runner IP while the same URLs returned 200 externally. User confirmed Astra-style motion, navbar/menu, admin spacing, sticky admin sidebar, security hardening and responsive menu on MacBook and Windows as the project UI/UX standard.
