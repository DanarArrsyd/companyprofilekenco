...
- Latest staging release is commit `b9d32a5` (proportional desktop scaling `e4af960` + Indonesian draft revision `4fef42c`); GitHub Actions run `36089960889` passed build, deploy, and smoke tests. Favicon: head links the Settings favicon, and Settings saves plus deploy (`favicon:publish --web-root`) copy it to the web root as a real `favicon.ico`, because Hostinger's edge serves `/favicon.ico` statically. The staging front controller calls `usePublicPath(public_html)`, so `public_path()` is the web root on staging. User confirmed Astra-style motion, navbar/menu, admin spacing, sticky admin sidebar, security hardening and responsive menu on MacBook and Windows as the project UI/UX standard.
