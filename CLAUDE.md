...
- Admin auto-translate: saving with the "Auto-translate the other language on save" checkbox (ContentLocaleTabs, stored in localStorage, sent as `X-Auto-Translate: 1` by `registerAutoTranslateHeader` in app.tsx) makes `AutoTranslateChanges` fill the other locale in `HasLocalizedContent`'s saving hook: English-only change → Indonesian regenerated, Indonesian-only → English regenerated, both → kept, untouched with empty Indonesian → filled. Engine: Azure Translator F0 via the `Translator` interface (`AZURE_TRANSLATOR_KEY`, `AZURE_TRANSLATOR_REGION`); no key → option hidden. Terms that must stay as written go in `config/translation.php` `glossary`. Failures keep the save and flash a warning. Tests reset `LocalizedContent` flags in `tests/TestCase.php`; fake Azure with `Http::fake()` (patterns match any host suffix). Page section `content` text is covered through `PageSection::translatableContentPaths()` (must match the `setText()` keys in `SectionContentFields.tsx`; `tests/js/section-content-paths.test.ts` checks it).
- Public footer (`PublicFooter.tsx`, user reference 2026-09-26): navy wave image `resources/img/element_footer.webp` (its navy = `navy-950`; the WebP is cropped 3 px from the PNG to drop transparent bottom rows) with the KMI mascot `footer_maskot1.webp` bottom-left (head above the wave, cut at the waist by the info strip on desktop, below the CTA on mobile), "Connect With Us" in Montserrat + social circles from Settings + "Contact Us" pill on the right, and a slim strip with quick links, address, email/phone and copyright. The content overlaps the wave by a vw margin (the image scales with viewport width); the navy wrapper is `flow-root` so that margin does not pull the background over the wave's arc and star.
...
- Latest staging release is commit `39b1b8a` (run `36167507088`: CSS audit, admin CRUD fixes and field hints, auto-translate phases 1–2, homepage ends after Latest News, Connect-With-Us footer). Auto-translate stays hidden until `AZURE_TRANSLATOR_KEY`/`AZURE_TRANSLATOR_REGION` are set in staging `application/.env` and a deploy re-caches config. Favicon: head links the Settings favicon, and Settings saves plus deploy (`favicon:publish --web-root`) copy it to the web root as a real `favicon.ico`, because Hostinger's edge serves `/favicon.ico` statically. The staging front controller calls `usePublicPath(public_html)`, so `public_path()` is the web root on staging. User confirmed Astra-style motion, navbar/menu, admin spacing, sticky admin sidebar, security hardening and responsive menu on MacBook and Windows as the project UI/UX standard.
